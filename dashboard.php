...
// Fetch room data for the logged-in user from roomstable
$stmt = $conn->prepare("SELECT r.* FROM roomstable r INNER JOIN rooms rm ON r.room = rm.name WHERE rm.user_id = ?");
...
<body>
            ...
            <table class="table">
                ...
                <tbody>
                    <?php while ($row = $result->fetch_assoc()): 
                        ...
                        $eco2Status = ($row['eCO2'] > 1000) ? 'status-danger' : (($row['eCO2'] > 800) ? 'status-warning' : 'status-normal');
                        $tvocStatus = ($row['TVOC'] > 500) ? 'status-danger' : (($row['TVOC'] > 250) ? 'status-warning' : 'status-normal');
                    ?>
                        <tr>
                            <td><?php echo htmlspecialchars($row['room']); ?></td>
                            <td><span class="<?php echo $tempStatus; ?>"></span><?php echo htmlspecialchars($row['temperature']); ?> °C</td>
                            <td><?php echo htmlspecialchars($row['luminosity']); ?> Lux</td>
                            <td><span class="<?php echo $humidityStatus; ?>"></span><?php echo htmlspecialchars($row['humidity']); ?>%</td>
                            <td><span class="<?php echo $eco2Status; ?>"></span><?php echo htmlspecialchars($row['eCO2']); ?> ppm</td>
                            <td><span class="<?php echo $tvocStatus; ?>"></span><?php echo htmlspecialchars($row['TVOC']); ?> ppb</td>
                        </tr>
                    <?php endwhile; ?>
                    <?php if ($result->num_rows === 0): ?>
                        <tr>
                            <td colspan="6" class="text-center">No rooms found for this user.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
    ...
    <script>
        ...
        function initMap() {
            // Replace with your building's coordinates
            const map = L.map('openstreetmap').setView([12.9716, 77.5946], 15); // Example: Bangalore, India
