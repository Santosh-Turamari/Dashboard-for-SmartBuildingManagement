...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_camera'])) {
    ...
    if ($stmt->execute()) {
        ...
        $_SESSION['error'] = "Failed to update camera: " . $conn->error;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_camera'])) {
    $name = $_POST['cameraName'];
    $location = $_POST['cameraLocation'];
    $type = $_POST['cameraType'];
    ...
    if ($stmt->execute()) {
        ...
        $_SESSION['error'] = "Failed to add camera: " . $conn->error;
    }
    header("Location: camera.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_cameras'])) {
    $camera_ids = json_decode($_POST['camera_ids']);
    if (!empty($camera_ids)) {
        ...
        if ($stmt->execute()) {
            ...
            $_SESSION['error'] = "Failed to delete cameras: " . $conn->error;
        }
    } else {
            ...
            <?php foreach ($cameras as $camera): ?>
                <div class="camera-card">
                    <div class="camera-preview">
                        ...
                        <?php endif; ?>
                        <span class="camera-status status-<?php echo strtolower($camera['status']); ?>"><?php echo $camera['status']; ?></span>
                            ...
                            <form method="POST">
                                <div class="modal-body">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" name="name" class="form-control" value="<?php echo htmlspecialchars($camera['name']); ?>" required>
                                    </div>
                                    <div class="mb-3">
                                        <label for="location" class="form-label">Location</label>
                                        <input type="text" name="location" class="form-control" value="<?php echo htmlspecialchars($camera['location']); ?>" required>
                                    </div>
                                    <div class="mb-3">
                                        <label for="status" class="form-label">Status</label>
                                        <select name="status" class="form-select" required onchange="updatePreview(this, <?php echo $camera['id']; ?>)">
    ...
    <script>
        ...
        function deleteSelectedCameras() {
            ...
            if (confirm(`Are you sure you want to delete ${selectedCameras.size} camera(s)?`)) {
                ...
                cameraIdsInput.value = JSON.stringify(Array.from(selectedCameras));
        ...
        function openAddCameraModal() {
            var modal = new bootstrap.Modal(document.getElementById('addCameraModal'));
            modal.show();
        }
        ...
        async function streamCamera(cameraId, videoElementId, cameraType) {
            ...
            try {
                ...
                if (videoDevices.length === 0) {
                    throw new Error('No video devices found. Please ensure a camera is connected.');
                }

                console.log('Available video devices:', videoDevices); // Debug: Log devices

                let deviceId;
                if (cameraType === 'built-in') {
                    // Try to find a device with "integrated" or "built-in" in its label
                    const builtInDevice = videoDevices.find(device => 
                        device.label.toLowerCase().includes('integrated') || 
                        device.label.toLowerCase().includes('built-in') ||
                        device.label.toLowerCase().includes('default')
                    ) || videoDevices[0]; // Fallback to first device if no match
                    deviceId = builtInDevice.deviceId;
                    console.log('Selected built-in camera:', builtInDevice);
                } else if (cameraType === 'usb') {
                    // Use cameraId to map to USB devices, excluding built-in
                    const builtInIndex = videoDevices.findIndex(device => 
                        device.label.toLowerCase().includes('integrated') || 
                        device.label.toLowerCase().includes('built-in') ||
                        device.label.toLowerCase().includes('default')
                    );
                    const usbDevices = builtInIndex !== -1 ? videoDevices.filter((_, i) => i !== builtInIndex) : videoDevices.slice(1);
                    const usbIndex = cameraId - 1; // Adjust based on cameraId
                    if (usbIndex >= usbDevices.length) {
                        throw new Error(`USB Camera #${cameraId} not found. Only ${usbDevices.length} USB devices available.`);
                    }
                    deviceId = usbDevices[usbIndex].deviceId;
                    console.log('Selected USB camera:', usbDevices[usbIndex]);
                }
                ...
                videoElement.parentElement.innerHTML += `<p class="text-danger">Unable to access camera: ${error.message}. Please check permissions and device availability.</p>`;
            }
        }
