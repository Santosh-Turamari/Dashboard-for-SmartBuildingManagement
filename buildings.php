...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['toggle_device'])) {
        ...
        switch ($currentState) {
            ...
            case 'Off':
                ...
                break;
            case 'LOCKED':
                $_SESSION['smart_home']['rooms'][$room]['devices'][$device] = 'UNLOCKED';
                break;
            case 'UNLOCKED':
                $_SESSION['smart_home']['rooms'][$room]['devices'][$device] = 'LOCKED';
                break;
            case 'OPEN':
    ...
    if (isset($_POST['media_action'])) {
        ...
        switch ($action) {
            case 'play_pause':
                $_SESSION['smart_home']['media']['playing'] = !$_SESSION['smart_home']['media']['playing'];
                break;
            case 'volume_up':
                $_SESSION['smart_home']['media']['volume'] = min($_SESSION['smart_home']['media']['volume'] + 5, 100);
                break;
            case 'volume_down':
                $_SESSION['smart_home']['media']['volume'] = max($_SESSION['smart_home']['media']['volume'] - 5, 0);
                break;
            case 'toggle_bluetooth':
                $_SESSION['smart_home']['media']['bluetooth_connected'] = !$_SESSION['smart_home']['media']['bluetooth_connected'];
                break;
        }
    }
...
$current = $data['rooms'][$data['current_room']];
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <div class="main-content">
        ...
        <div class="row g-4 mb-4">
            <div class="col-md-4">
                <div class="info-box">
                    ...
                    <div class="info-value"><?php echo $current['temperature']; ?>°c</div>
                    <div class="progress mt-2" style="height: 8px;">
                        <div class="progress-bar bg-warning" role="progressbar" style="width: <?php echo min($current['temperature'] * 3, 100); ?>%"></div>
                    </div>
                </div>
            </div>
            <div class="col-md-4">
                <div class="info-box">
                    ...
                    <div class="info-value"><?php echo $current['humidity']; ?>%</div>
                    <div class="progress mt-2" style="height: 8px;">
                        <div class="progress-bar bg-info" role="progressbar" style="width: <?php echo $current['humidity'] * 2; ?>%"></div>
                    </div>
                </div>
            </div>
            <div class="col-md-4">
        ...
        <div class="controller-box">
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <span class="badge bg-primary"><?php echo count($current['devices']); ?> Devices</span>
            </div>
            
            <div class="device-status">
                <?php foreach ($current['devices'] as $device => $status): 
                    $isBinary = in_array($status, ['On', 'Off', 'LOCKED', 'UNLOCKED', 'OPEN', 'CLOSED']);
                    $isActive = in_array($status, ['On', 'UNLOCKED', 'CLOSED']);
                            ...
                            <form method="POST" class="w-100">
                                ...
                                <button type="submit" name="set_room" class="btn p-0 text-start w-100">
                                    ...
                                    <div class="d-flex justify-content-between">
                                        <span><?php echo $roomData['temperature']; ?>°C</span>
        ...
        <div class="media-controls">
            <div class="media-player">
                <div class="media-info">
                    <div class="station-name"><?php echo $data['media']['station']; ?></div>
                    <div class="volume-display">Volume: <?php echo $data['media']['volume']; ?>%</div>
                </div>
                <div class="media-control-buttons">
                    <form method="POST">
                        <button type="submit" name="media_action" value="play_pause" class="media-btn play-pause">
                            <i class="fas fa-<?php echo $data['media']['playing'] ? 'pause' : 'play'; ?>"></i>
                        </button>
                    </form>
                </div>
                <div class="volume-control">
                    <form method="POST">
                        <button type="submit" name="media_action" value="volume_down" class="media-btn">
                            <i class="fas fa-volume-down"></i>
                        </button>
                    </form>
                    <div class="volume-slider">
                        <div class="progress">
                            <div class="progress-bar" role="progressbar" style="width: <?php echo $data['media']['volume']; ?>%" 
                                 aria-valuenow="<?php echo $data['media']['volume']; ?>" aria-valuemin="0" aria-valuemax="100"></div>
                        </div>
                    </div>
                    <form method="POST">
                        <button type="submit" name="media_action" value="volume_up" class="media-btn">
                ...
                <div class="bluetooth-status">
                    <form method="POST">
                        <button type="submit" name="media_action" value="toggle_bluetooth" 
                                class="btn btn-sm bluetooth-btn <?php echo $data['media']['bluetooth_connected'] ? 'btn-success' : 'btn-secondary'; ?>">
                            <i class="fas fa-bluetooth-b"></i>
                            <?php echo $data['media']['bluetooth_connected'] ? 'Connected' : 'Connect'; ?>
                        </button>
                    </form>
                </div>
            </div>
    ...
    <script>
        ...
        function toggleDeleteMode() {
            ...
            if (isDeleteMode) {
                ...
                var modal = new bootstrap.Modal(document.getElementById('deleteOptionsModal'));
                modal.show();
            } else {
        ...
        function openAddOptionsModal() {
            var modal = new bootstrap.Modal(document.getElementById('addOptionsModal'));
